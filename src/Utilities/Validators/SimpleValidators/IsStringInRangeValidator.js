...
export class IsStringInRangeValidator extends AbstractValidator
{
	...
	constructor( minValue, maxValue, isInclusive )
	{
		...
		if ( null !== minValue && null !== maxValue && true === minValue.isGreaterThan( maxValue ) )
		{
				...
				null === this.#_minValue
					...
					: (
						false === this.#_isInclusive
							? true === value.isGreaterThan( this.#_minValue )
							: true === value.isGreaterThanOrEqualTo( this.#_minValue )
					)
			)
			&& (
				null === this.#_maxValue
					...
					: (
						false === this.#_isInclusive
							? true === value.isLowerThan( this.#_maxValue )
							: true === value.isLowerThanOrEqualTo( this.#_maxValue )
					)
			);
