'use strict';

import { InvalidPropertyException } from '../../../Types/InvalidPropertyException.js';
import { IsUnsignedIntegerValidator } from '../SimpleValidators/IsUnsignedIntegerValidator.js';
import { ObjectPropertyWithInnerValidatorValidator } from './ObjectPropertyWithInnerValidatorValidator.js';

/**
 * Represents an object validator validating if an object's property is an unsigned integer.
 * @author Christian Ramelow <info@codekandis.net>
 */
export class ObjectPropertyIsUnsignedIntegerValidator extends ObjectPropertyWithInnerValidatorValidator
{
	/**
	 * Constructor method.
	 * @param {Object} object The object to validate.
	 * @param {String} propertyName The name of the object's property to validate.
	 * @throws {InvalidPropertyException} The object's property does not exist.
	 */
	constructor( object, propertyName )
	{
		super(
			object,
			propertyName,
			new IsUnsignedIntegerValidator()
		);
	}

	/**
	 * @inheritdoc
	 * Validates if the object's property is an unsigned integer.
	 */
	validate()
	{
		return super.validate();
	}
}
